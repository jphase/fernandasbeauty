		...
		<div class="container">
			<div class="row">
				<div class="col-xs-12">
					...
					<?php
						$cpt            = 'cpt_service';
						$cpt_taxonomy   = 'service-category';
						$masonry        = get_post_meta( get_the_ID(), 'services_listing_masonry', true );
						$isotope        = get_post_meta( get_the_ID(), 'services_listing_isotope', true );
						$columns        = get_post_meta( get_the_ID(), 'services_listing_columns', true );
						$posts_per_page = get_post_meta( get_the_ID(), 'services_listing_posts_per_page', true );
					...
					<?php if ( 'on' == $isotope ): ?>
						<ul class="filters-nav group">
							<li><a href="#filter" class="selected btn small transparent" data-filter="*"><?php _e( 'All Services', 'ci_theme' ); ?></a></li>
							<?php $cats = get_terms( $cpt_taxonomy, array( 'hide_empty' => 1 ) ); ?>
							<?php foreach ( $cats as $cat ): ?>
								<li><a href="#filter" class="btn small transparent" data-filter=".<?php echo esc_attr( $cat->slug ); ?>"><?php echo $cat->name; ?></a></li>
							<?php endforeach; ?>
						</ul>
					<?php endif; ?>
