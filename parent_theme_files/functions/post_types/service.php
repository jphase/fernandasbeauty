...
function ci_create_cpt_service() {
	...
	$args = array(
		'labels'          => $labels,
		'singular_label'  => _x( 'Service', 'post type singular name', 'ci_theme' ),
		'public'          => true,
		'show_ui'         => true,
		'capability_type' => 'post',
		'hierarchical'    => false,
		'has_archive'     => _x( 'services-archive', 'post type archive slug', 'ci_theme' ),
...
add_action( 'load-post.php', 'ci_cpt_service_meta_boxes_setup' );
add_action( 'load-post-new.php', 'ci_cpt_service_meta_boxes_setup' );

if ( ! function_exists( 'ci_cpt_service_meta_boxes_setup' ) ):
function ci_cpt_service_meta_boxes_setup() {
	add_action( 'add_meta_boxes', 'ci_cpt_service_add_meta_boxes' );
	add_action( 'save_post', 'ci_cpt_service_save_meta', 10, 2 );
}
endif;

if ( ! function_exists( 'ci_cpt_service_add_meta_boxes' ) ):
function ci_cpt_service_add_meta_boxes() {
	add_meta_box( 'service-box', __( 'Service Settings', 'ci_theme' ), 'ci_cpt_service_meta_box', 'cpt_service', 'normal', 'high' );
}
endif;

if ( ! function_exists( 'ci_cpt_service_meta_box' ) ):
function ci_cpt_service_meta_box( $object, $box ) {
	ci_prepare_metabox( 'cpt_service' );

	ci_metabox_input( 'ci_cpt_service_price', esc_html__( 'Price', 'ci_theme' ) );
	...
	?><div class="ci-cf-wrap"><?php
		ci_metabox_open_tab( __('Information', 'ci_theme') );
			...
			<fieldset class="ci-repeating-fields">
				<div class="inner">
					<?php
						$fields = get_post_meta( $object->ID, 'ci_cpt_service_fields', true );

						if ( ! empty( $fields ) ) {
							foreach ( $fields as $field ) {
								?>
								<div class="post-field">
									<label><?php _e( 'Title:', 'ci_theme' ); ?> <input type="text" name="ci_cpt_service_fields_repeatable_title[]" value="<?php echo esc_attr( $field['title'] ); ?>" class="widefat" /></label>
									<label><?php _e( 'Description:', 'ci_theme' ); ?> <input type="text" name="ci_cpt_service_fields_repeatable_description[]" value="<?php echo esc_attr( $field['description'] ); ?>" class="widefat" /></label>
					...
					<div class="post-field field-prototype" style="display: none;">
						<label><?php _e( 'Title:', 'ci_theme' ); ?> <input type="text" name="ci_cpt_service_fields_repeatable_title[]" value="" class="widefat" /></label>
						<label><?php _e( 'Description:', 'ci_theme' ); ?> <input type="text" name="ci_cpt_service_fields_repeatable_description[]" value="" class="widefat" /></label>
						<p class="ci-repeating-remove-action"><a href="#" class="button ci-repeating-remove-field"><i class="dashicons dashicons-dismiss"></i><?php _e( 'Remove me', 'ci_theme' ); ?></a></p>
					</div>
				</div>
...
if ( ! function_exists( 'ci_cpt_service_save_meta' ) ):
function ci_cpt_service_save_meta( $post_id, $post ) {

	if ( !ci_can_save_meta('cpt_service') ) return;

	update_post_meta( $post_id, 'ci_cpt_service_price', esc_html( $_POST['ci_cpt_service_price'] ) );
	update_post_meta( $post_id, 'ci_cpt_service_fields', ci_theme_sanitize_service_fields_repeating( $_POST ) );
	update_post_meta( $post_id, 'meta_placement', in_array( $_POST['meta_placement'], array( 'left', 'right' ) ) ? $_POST['meta_placement'] : '' );
}
endif;

if ( ! function_exists( 'ci_theme_sanitize_service_fields_repeating' ) ) :
function ci_theme_sanitize_service_fields_repeating( $POST_array ) {
	if ( empty( $POST_array ) || !is_array( $POST_array ) ) {
		return false;
	}

	$titles       = $POST_array['ci_cpt_service_fields_repeatable_title'];
	$descriptions = $POST_array['ci_cpt_service_fields_repeatable_description'];
